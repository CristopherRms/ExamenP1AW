...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [RouterOutlet, NgFor, ShowComponent, FormsModule,NgIf, ReactiveFormsModule, FormComponent,FiltrosComponent,ModificarComponent],
...
export class AppComponent {
    ...
    public showEditando: Show | null = null;
    ...
    public deleteElem(name: string): void {
      this.shows = this.shows.filter(show => show.name != name);
    }
  
    public filtrado(tipo: string): void {
      if (tipo == "A") {
        this.alphabeticSort();
      }
      if (tipo == "B") {
